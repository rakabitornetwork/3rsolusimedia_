...
export default function Form({
    ...
    const loadProfiles = async (id) => {
        ...
        try {
            const response = await fetch(
                `/admin/customers/hotspot/profiles?router_id=${encodeURIComponent(id)}`,
                {
                    headers: {
    ...
    const submit = (e) => {
        e.preventDefault();
        if (editing) {
            put(`/admin/customers/hotspot/service-profiles/${item.id}`);
        } else {
            post('/admin/customers/hotspot/service-profiles');
        }
    };

    return (
        <AdminLayout
            title={editing ? 'Edit Profile Layanan Hotspot' : 'Tambah Profile Layanan Hotspot'}
            subtitle="Hubungkan nama paket ke Hotspot profile di MikroTik"
        >
            <Head title={editing ? 'Edit Profile Layanan Hotspot' : 'Tambah Profile Layanan Hotspot'} />
            ...
            >
                ...
                <label className="inline-flex items-center gap-2 text-sm text-ink">
                    ...
                    />
                    Aktifkan Profile Layanan Hotspot
                </label>
                ...
                <div className="flex flex-wrap gap-3 pt-2">
                    ...
                    >
                        {processing ? 'Menyimpan...' : editing ? 'Simpan Perubahan' : 'Simpan Profile'}
                    </button>
                    <Link
                        href="/admin/customers/hotspot/service-profiles"
